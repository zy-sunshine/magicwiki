...
class APCImages {
	...
	public static function generateImage( $type, IContextSource $context ) {
		// @todo FIXME: Nasty global...
		global $col_black;
			...
			case 1:
				...
				foreach ( $string_placement as $angle ) {
					self::text_arc(
						...
						$y,
						$size,
						$angle[0] * 360, $angle[1] * 360,
						$col_black,
						$context->getLanguage()->formatSize( $s * ( $angle[1] - $angle[0] ) )
					);
				}
				break;
			case 2:
			...
			case 3:
				...
				for ( $i = 0; $i < $mem['num_seg']; $i++ ) {
					...
					foreach ( $free as $block ) {
						if ( $block['offset'] != $ptr ) { // Used block
							...
							if ( $h > 0 ) {
								$j++;
								if ( $j < 75 ) {
									self::fill_box( $image, $x, $y, 50, $h, $col_black, $col_red, $lang->formatSize( $block['offset'] - $ptr ), $j );
								} else {
									self::fill_box( $image, $x, $y, 50, $h, $col_black, $col_red );
								}
							}
							$y += $h;
						}
						...
						if ( $h > 0 ) {
							$j++;
							if ( $j < 75 ) {
								self::fill_box( $image, $x, $y, 50, $h, $col_black, $col_green, $lang->formatSize( $block['size'] ), $j );
							} else {
								self::fill_box( $image, $x, $y, 50, $h, $col_black, $col_green );
							}
						}
						$y += $h;
						$ptr = $block['offset'] + $block['size'];
					...
					if ( $ptr < $mem['seg_size'] ) { // memory at the end
						...
						if ( $h > 0 ) {
							self::fill_box( $image, $x, $y, 50, $h, $col_black, $col_red, $lang->formatSize( $mem['seg_size'] - $ptr ), $j++ );
						}
					}
				}
				break;
